...
export default function DeliverymanDashboard() {
  ...
  return (
    <div>
      <BrandBar name={user?.name ?? "Rider"} welcome={`Welcome, ${user?.name?.split(" ")[0] ?? "Rider"}`} />
        ...
        <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-5 text-white shadow-soft">
          ...
          <div className="flex gap-4 mt-4">
            <div className="flex-1 bg-white/15 rounded-xl px-3 py-2">
              <p className="text-[11px] text-emerald-50 flex items-center gap-1.5"><Icon name="cash" size={14} /> Cash</p>
              <p className="font-bold">{inr(cash)}</p>
            </div>
            <div className="flex-1 bg-white/15 rounded-xl px-3 py-2">
              <p className="text-[11px] text-emerald-50 flex items-center gap-1.5"><Icon name="online" size={14} /> Online</p>
                ...
                <Link key={o.id} href={`/deliveryman/orders/${o.id}`}>
                  <Card className="p-3.5 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400 truncate flex items-center gap-1.5"><Icon name="pin" size={13} /> {o.location.address}</p>
                    </div>
